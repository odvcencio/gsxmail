// Package importer implements `gsxmail import`: it parses an existing
// email HTML file — MJML compiled output, react-email output, or
// hand-written table
...
// walkable tree instead of a hard failure. gsxmail's own render path (renderhtml, doc, lower, gsxmail.go) never
...
// "it just works, then you tune it" promise is the whole point, not a
// nice-to-have — see the README's "Import from existing HTML" section.
package importer
